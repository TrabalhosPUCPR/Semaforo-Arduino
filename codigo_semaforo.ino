...
void ligarLedPed(int sinaleiro){
  digitalWrite(leds_pedestre[sinaleiro-1], HIGH);
}
void desligarLedPed(int sinaleiro){
  digitalWrite(leds_pedestre[sinaleiro-1], LOW);
}
...
void pisca_led_ped(int sinaleiro, int tempo){ // inclui o delay()
  ...
  while(millis() - temp_comeco <= tempo){
    ...
    ligarLedPed(sinaleiro);
    delay(intervalo);
    if(millis() - temp_comeco >= tempo*0.4){
      desligarLedPed(sinaleiro);
      delay(intervalo);
    }
  }
  desligarLedPed(sinaleiro);
